...
E_cut_wfc=32	# Zdefiniowanie energii kinetycznych odcięcia dla funkcji falowej
N_PROC_TAB=(1 2 3 4)		# Zdefiniowanie ilości wykorzystywanych procesorów  

echo '' > time_procek.txt

Epsi=$E_cut_wfc

calc_name_prefix='Si.scf.'$Epsi		# Zdefiniowanie prefixu obliczeń
input_file=$calc_name_prefix'_2_.in'	# Zdefiniowanie nazwy pliku wsadowego dla programu pw.x z pakietu QE 
output_file=$calc_name_prefix'_2_.out'	# Zdefiniowanie pliku z wynikami
echo $calc_name_prefix

Erho=$((8*$Epsi))			# Obliczenie warości energii kinetycznej odcięcia dla gęstości łądunku - liniowa zależność 

for N_PROC in "${N_PROC_TAB[@]}";
do
...
grep "PWSCF        :" $output_file | cut -d":" -f2 | cut -d's' -f1 >> time_procek.txt
